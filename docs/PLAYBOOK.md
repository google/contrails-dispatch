...
### The `pycontrails` pipeline

To stack-rank flights by their potential warming impact, your IT team can set up an integration built around the open-source [**pycontrails**](https://github.com/contrailcirrus/pycontrails) library and the **Google Contrails Forecast API**. Here is how the process works:
...
2. **Data enhancement:** The pipeline parses and resamples the planned 4D flight path (latitude, longitude, altitude, and time) using `pycontrails`, fetches high-resolution meteorological forecast grids from the Google Contrails API (`getGrids` via `pycontrails.datalib.google_forecast`), and intersects the trajectory with the forecast grid to predict where persistent contrails are likely to form.
3. **Scoring and ranking:** The pipeline calculates the forecasted contrail impact score—measured as total expected effective energy forcing in Joules (`total_forcing_joules`)—for every single flight. Planners can then use [**Contrails Explorer**](https://contrails.webapps.google.com/), a visual interface, to review the specific contrail forecast for high-impact flights.
4. **Data analysis:** Dispatchers can then view this enriched data in their preferred dashboard. By sorting the daily schedule by contrail impact, dispatchers can easily identify the flights with the highest warming potential. For operational trials, airlines employ different strategies; most do not attempt avoidance on every flight, but rather select an appropriate number of high-impact flights to test based on their specific trial needs and operational capacity.
...
* **Input the flight:** Enter the flight plan details into the Contrails Explorer interface.
...
This section describes a cloud-based solution for airline partners to ingest flight plans, calculate their contrail warming impact using `pycontrails` and Google Contrails Forecast data, and store the results in BigQuery for stack-ranking and trial selection analysis.
...
* [**Cloud Functions (2nd gen)**](https://cloud.google.com/functions): A serverless, event-driven Functions-as-a-Service (FaaS) model built on Cloud Run. It is used for the Pub/Sub-triggered process that parses the XML, fetches the Google Contrails Forecast grid via `pycontrails`, and calculates the expected effective energy forcing, offering a cost-effective alternative to Dataflow, with improved concurrency and longer timeouts.
...
2. **Process**: **Cloud Function v2** parses the XML and computes contrail energy forcing using **`pycontrails`** in a few seconds.
...
The pipeline includes a specialized parser designed for **ARINC 633 Operational Flight Plan (OFP) XML** flight plans. This engine extracts waypoint-level information such as latitude, longitude, altitude, and time for the main flight plan. An open source version is available in the [pycontrails](https://github.com/contrailcirrus/pycontrails) library (`pycontrails.core.flightplan.parse_ofp_xml`).

This pre-processing ensures that raw operational data is ready for evaluation without requiring the partner to modify their existing flight planning exports or spending engineering work on creating the parser from scratch.

#### Evaluating impact with `pycontrails`

The pipeline in [`contrails_dispatch/flight_plan_processor/main.py`](../contrails_dispatch/flight_plan_processor/main.py) uses `pycontrails` to parse the flight plan, retrieve Google Contrails forecast grids (via `pycontrails.datalib.google_forecast`, which calls the Contrails [GetGrids API](https://developers.google.com/contrails/reference/v2/grids/getGrids)), and compute the total expected effective energy forcing (`total_forcing_joules`):
...
# 1. Parse the ARINC 633 OFP XML and resample waypoints
flight = flightplan.parse_ofp_xml(pubsub_message)
flight = flight.resample_and_fill()

# 2. Fetch Google Contrails Forecast grid data for the flight time window
...
# 3. Intersect the flight trajectory with the forecast grid and compute total forcing (Joules)
...
Each processed row is saved in `APPEND_ONLY` mode into a historical BigQuery table ([`contrails_impact_history.json`](../infrastructure/bigquery/schema/contrails_impact_history.json)). This schema captures essential data elements such as the commercial flight number, origin and destination ICAO codes, aircraft type and registration, planned departure and arrival timestamps, serialized flight plan JSON, and the calculated `total_forcing_joules`.

| field name | mode | type | description |
| ----- | ----- | ----- | ----- |
| flight_commercial_number | NULLABLE | STRING | The alphanumeric code assigned by an airline to a scheduled flight (e.g., CA123). |
| departure_airport_icao | NULLABLE | STRING | The ICAO code for the airport where the flight originates (e.g., KJFK). |
| arrival_airport_icao | NULLABLE | STRING | The ICAO code for the airport where the flight concludes (e.g., LSZH). |
| aircraft_type | NULLABLE | STRING | The ICAO code for the aircraft type (e.g., A35K). |
| aircraft_registration | NULLABLE | STRING | The unique registration identifier of the aircraft (tail number, e.g., B-LPP). |
| origin_date | NULLABLE | DATE | The date on which the flight is scheduled to begin. |
...
| last_updated_at | NULLABLE | TIMESTAMP | The timestamp indicating when the flight plan was last modified. |
| total_forcing_joules | NULLABLE | FLOAT | The total expected effective energy forcing of the forecasted contrails impact in Joules. |
| flight_plan_json | NULLABLE | STRING | The parsed flight plan stored as a JSON string. |
| bq_last_update_timestamp | NULLABLE | TIMESTAMP | The timestamp indicating when the record was last updated in BigQuery. |
...
To support your flight planners, a BigQuery View ([`contrails_impact.sql`](../infrastructure/bigquery/views/contrails_impact.sql)) filters for the *latest version* of each flight plan. It is possible to use this view to power an up-to-date, live dashboard that stack-ranks flights based on their estimated contrail warming impact.
